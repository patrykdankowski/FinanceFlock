...
class ExpenseGeolocationServiceAdapter implements ExpenseGeolocationServicePort {
    ...
    public String getLocationFromUserIp(String userIp) {
        ...
        ResponseEntity<InfrastructureExpenseDto> response = restTemplate.getForEntity(apiUrl, InfrastructureExpenseDto.class);

        InfrastructureExpenseDto geolocationDto = response.getBody();
    ...
    @Override
    public ExpenseDtoWriteModel validateAndPrepareExpense(final ExpenseDtoWriteModel expenseDtoWriteModel,
                                                          final String userIp) {
